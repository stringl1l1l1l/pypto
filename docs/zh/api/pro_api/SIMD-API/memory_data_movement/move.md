...
- UB → L1 Buffer 仅执行按字节物理拷贝，不做layout转换。源UB Tile为ND时，禁止直接用layout=NZ/ZN的L1 Tile承接，会产生错误分形数据。必须先通过pypto_pro.language.move在UB内将ND转换为NZ（UB → UB），再写入L1 Buffer的NZ/ZN Tile，正确用法参见[UB数据转置写入L1 Buffer](#ub数据转置写入l1-buffer)调用示例。
