...
返回dst目的操作数，[reg_tensor](../basic_data_structures/reg_tensor.md)，支持的数据类型请参见[约束说明](#约束说明)。NORM模式下，当dst为16位宽数据类型（DT_INT16、DT_UINT16、DT_FP16、DT_BF16），src为8位宽数据类型（DT_INT8、DT_UINT8）时，目的操作数的低8位与源操作数相同，高8位自动补0。
