...
| 参数 | 输入/输出 | 说明 |
|---|---|---|
| container | 输入 | 目的操作数，Tile或Tensor类型，向其中写入单个元素。支持可参与标量表达式的整型或浮点类型，不支持DT_FP4、DT_FP8E4M3FN、DT_FP8E5M2、DT_INT4、DT_UINT4、DT_HF4、DT_HF8等仅用于存储的低精度类型。写入值类型须与元素类型一致或可由前端按该元素类型构造。 |
