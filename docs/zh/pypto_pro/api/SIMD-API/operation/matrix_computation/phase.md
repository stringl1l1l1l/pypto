...
`matmul` / `matmul_acc`的`phase`参数（`pl.AccPhase`）与`store` / `store_tile` / `move`的`phase`参数（`pl.STPhase`）共同控制Cube（矩阵乘）与（L0C→GM/L0C→UB搬运）之间的**unit_flag硬件握手**。正确使用phase可以省去软件同步、提升流水并行度；使用不当则会导致精度问题或设备卡死。
